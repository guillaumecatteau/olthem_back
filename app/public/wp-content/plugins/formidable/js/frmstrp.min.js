...
return queryForNameFieldIsFound?window.frmProForm.queryForNameField(fieldID,"field",$form[0]):$form[0].querySelector(`#frm_field_${fieldID}_container input, input[name="item_meta[${fieldID}]"], .frm_field_${fieldID}_container input`)}if(firstNameID!==""){const firstFieldContainer=getNameFieldItem(firstNameID,"container");if(firstFieldContainer?.querySelector(".frm_combo_inputs_container"))cardObject.name=getNameFieldValue(firstFieldContainer,"first");else{const firstField=getNameFieldItem(firstNameID,
"field",$form);if(firstField?.value)cardObject.name=firstField.value}}if(lastNameID!==""){const lastFieldContainer=getNameFieldItem(lastNameID,"container");if(lastFieldContainer?.querySelector(".frm_combo_inputs_container"))cardObject.name=`${cardObject.name} ${getNameFieldValue(lastFieldContainer,"last")}`;else{const lastField=getNameFieldItem(lastNameID,"field",$form);if(lastField?.value)cardObject.name=`${cardObject.name} ${lastField.value}`}}return cardObject}function submitForm(){if(running>
